...
/* Fleet Console components — ported from
   CarPapi Fleet _ Design System/ui_kits/fleet-console/components.jsx.
   All inline styles use the CSS vars scoped to .fleet-page (see
   src/styles/fleet-tokens.css). Animation names are prefixed
   `fleet-` to avoid collision with the rest of the app. */

/* ------------ MeterBar ------------ */
export function MeterBar({ value, max, color, label, right }) {
...
export function StatusLED({ status }) {
  ...
  return (
    <span style={{
      ...
      animation: map.pulse ? `fleet-blink-led ${map.speed} steps(2,end) infinite` : "none",
    }} />
  );
}

/* ------------ Tier chip ------------ */
export function TierChip({ tier, label }) {
...
/* ------------ TierRow ------------ */
export function TierRow({ id, label, count, color, active, onClick }) {
  return (
...
/* ------------ FleetSidebar (renamed from Sidebar to avoid clash) ------------ */
export function FleetSidebar({ tiers, activeTier, setActiveTier, summary }) {
  return (
    <aside style={{
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <TierRow id="ALL" label="ALL TIERS" count={tiers.reduce((n, t) => n + t.count, 0)} color="var(--accent)" active={activeTier==="ALL"} onClick={() => setActiveTier("ALL")} />
          {tiers.map(t => (
...
/* ------------ HudStat ------------ */
export function HudStat({ label, value, color }) {
  return (
...
/* ------------ FleetHeader (renamed from Header to avoid clash) ------------ */
export function FleetHeader({ clock, onSummon, onBack, agentCount }) {
  return (
    <header style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        {onBack && (
          <button onClick={onBack} className="px-btn" style={{ fontSize: 10 }}>◀ HOME</button>
        )}
        <div>
          <div className="t-callsign" style={{ color: "var(--accent)", fontSize: 10 }}>MISSION CONTROL · ROSTER</div>
          <h1 style={{ color: "var(--fg-1)", marginTop: 4, font: "var(--t-h1)", fontSize: 18 }}>{agentCount ?? 14} OPERATORS DEPLOYED</h1>
        </div>
      </div>
...
/* ------------ LogFeed ------------ */
export function LogFeed({ events }) {
  ...
  return (
    <div style={{
      ...
      <div ref={ref} style={{ flex: 1, overflowY: "auto", padding: "10px 12px", font: "var(--t-body)" }}>
        ...
        <div style={{ color: "var(--ok)" }}>&gt; <span style={{ animation: "fleet-blink-led 0.8s steps(2,end) infinite" }}>_</span></div>
      </div>
    </div>
  );
}
...
/* ------------ ScheduleStrip ------------ */
export function ScheduleStrip({ events, nowFrac }) {
  return (
...
/* ------------ AgentDialog ------------ */
export function AgentDialog({ agent, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        background: "var(--bg-3)",
        border: `2px solid ${tierVar}`,
        boxShadow: `0 0 0 2px var(--bg-0), var(--px-shadow-3)`,
      }}>
...
/* ------------ SummonPalette ------------ */
export function SummonPalette({ open, onClose, onSubmit }) {
